...
**Aggregated statistics (groundwork).** `features.hygeia.limits.maxStatsPeriodDays` (default `30`, editable from the config panel) caps the window a statistics query may span. The effective window is always the smaller of that cap and `retentionDays` — past the retention window there is no data left to aggregate — and a longer request is clipped and flagged as clipped, never rejected nor silently shortened. The statistics endpoints themselves (per-asset summary, per-tag aggregation, fleet ranking) arrive in later phases; the shared layer they build on is in place: a closed registry of the eight denormalized `AssetSnapshot` metrics (`cpuPct` … `powerWatts`, where a missing value stays missing instead of becoming zero), a pure summary (min / max / mean / p95 / current, with the timestamps of the extremes) and multi-asset snapshot queries that resolve any number of assets in a single SQL statement.
