...
| `POST` | `/hygeia/documents` | `HYGEIA_READ` | Request a document generated in the background: `kind=stats-csv` (a statistics table as CSV — `dataset` = `summary`/`tag-stats`/`ranking`/`overview` plus that scope's fields) or `kind=inventory-pdf` (the asset inventory as PDF — `scope` = `user`/`organization`, `includeSoftware`); answers `202` with the document `pending` |
...
| `hygeia.report` | Hygeia | `HygeiaDocumentManager.execute_document_generation` (statistics CSV, inventory PDF) | `hygeia-doc:<id>` |
