      ...
      <div class="modal-box">
        ...
        <form @submit.prevent="handleSubmit">
          <div class="modal-section">
            <h4>{{ t('users.create.personal') }}</h4>
            <div class="form-row">
              <div class="form-group"><label for="cu-first">{{ t('users.fields.firstName') }}</label><input id="cu-first" v-model="form.first_name" type="text" required class="inp" /></div>
              <div class="form-group"><label for="cu-last">{{ t('users.fields.lastName') }}</label><input id="cu-last" v-model="form.last_name" type="text" required class="inp" /></div>
            </div>
          </div>

          <div class="modal-section">
            <h4>{{ t('users.create.credentials') }}</h4>
            <div class="form-row">
              <div class="form-group"><label for="cu-username">{{ t('users.fields.username') }}</label><input id="cu-username" v-model="form.username" type="text" required class="inp" /></div>
              <div class="form-group"><label for="cu-email">{{ t('users.fields.email') }}</label><input id="cu-email" v-model="form.email" type="email" required class="inp" /></div>
            </div>
            <div class="form-group" style="margin-top:0.65rem">
              <label for="cu-password">{{ t('users.fields.password') }}</label>
              <div class="password-wrap">
                <input :id="'cu-password'" v-model="form.password" :type="pwVisible ? 'text' : 'password'" required class="inp" :placeholder="t('users.create.passwordPlaceholder')" />
          ...
          <div v-if="auth.isRoot" class="modal-section">
            <h4>{{ t('users.fields.role') }}</h4>
            <div class="form-group"><select v-model="form.role" class="inp select"><option value="role_user">{{ t('users.roles.user') }}</option><option value="role_admin">{{ t('users.roles.admin') }}</option></select></div>
          </div>
          ...
          <div class="modal-actions">
            ...
            <button type="submit" class="btn btn--primary" :disabled="submitting">{{ submitting ? t('login.register.submitting') : t('users.create.submit') }}</button>
          </div>
        </form>
      </div>
    </div>
...
const strengthLabel = computed(() => (strengthLevel.value ? t(`users.strength.${strengthLevel.value}`) : ''))
...
async function handleSubmit() { errorMsg.value = ''; if (form.password.length < 8) { errorMsg.value = t('users.create.passwordTooShort'); return }; submitting.value = true; emit('created', { ...form }) }
defineExpose({ reset })
</script>
